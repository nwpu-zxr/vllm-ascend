...
class KVTransferThread(threading.Thread):
    ...
    def _get_cache_roles(self, kv_cache_group_id: int = 0) -> list[str]:
        get_cache_roles = getattr(self.token_database, "get_cache_roles", None)
        if get_cache_roles is None:
            return ["kv"]
        return get_cache_roles(kv_cache_group_id)
    ...
    def _handle_request(self, req_meta: ReqMeta):
        ...
        try:
            ...
            for group_id in req_meta.kv_cache_group_ids or [0]:
                for cache_role in self._get_cache_roles(group_id):
                    starts = []
                    ends = []
                    keys = []
                    block_hashes = []
                    ...
                    for start, end, key, block_id in self._process_tokens_with_block_ids(
                        ...
                        skip_null_blocks=self._skip_null_blocks(req_meta, group_id, cache_role),
                        cache_role=cache_role,
                    ):
                    ...
                    logger.info(
                        "Storing KV cache for %d out of %d blocks (missing_count=%d) "
                        "for request %s in group %d role %s",
                        len(keys),
                    ...
                    logger.debug(
                        "KV pool put request=%s group=%d role=%s token_len=%d keys=%d sample_keys=%s",
                        req_id,
                        group_id,
                        cache_role,
                        token_len,
                    ...
                    for index, start in enumerate(starts):
                        ...
                        if self.enable_kv_event and cache_role == "kv":
                            token_ids = (
                                req_meta.token_ids[start : ends[index]] if req_meta.token_ids is not None else None
                            )
                            block_size = (
